...
export default function Profile({ user, userLoggedIn }: any) {

    const dispatch = useDispatch();

    const profileAction = useSelector((state: any) => state.profile);

    const history = useHistory();
    const [avatar, setAvatar] = useState("");

    const enableEmail = () => {
        (document.querySelector('input[name="email"]') as HTMLInputElement).disabled = false;
    };
            ...
            <div className="block-profile-content profile">
                ...
                <Formik 
                    ...
                    onSubmit={async (values: any) => {
                        ...
                        const result: any = await dispatch(userService.updateProfile(values, userLoggedIn?.user.localId));
