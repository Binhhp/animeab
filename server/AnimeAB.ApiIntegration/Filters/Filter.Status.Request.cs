            ...
            else
            {
                if (rootStatus.Contains(":") && (rootStatus.Contains("lt") || rootStatus.Contains("gt")))
                {
                    if (rootStatus.Contains("lt") && rootStatus.Contains(":"))
                    {
                        ...
                        if (status == 3) 
                            animes = animes.Where(x => x.IsStatus < status 
                                    || (x.DateRelease.Month > DateTime.Now.Month - 4 && x.DateRelease.Year == DateTime.Now.Year))
                                .ToList();

                        else animes = animes.Where(x => x.IsStatus < status).ToList();
                    }
